...
services:
  ...
  tengine:
    ...
    networks:
      - safeline-ce
      - net_traefik
#    network_mode: host # default configuration
  luigi:
...
```toml
[Unit]
Description=Docker Safeline WAF
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/bin/bash -c "docker compose -f /docker/safeline/compose.yaml up --detach"
ExecStop=/bin/bash -c "docker compose -f /docker/safeline/compose.yaml stop"
